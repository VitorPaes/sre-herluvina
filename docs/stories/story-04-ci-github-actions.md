...
- [ ] **Etapa 1: Estruturação e Triggers do Workflow**
  - [ ] Criar o diretório `.github/workflows/`
  - [ ] Criar o arquivo de especificação `.github/workflows/ci.yml`
  - [ ] Configurar os gatilhos (triggers) do workflow para eventos de `push` e `pull_request` nas branches `main` e `feature/*`
- [ ] **Etapa 2: Implementação dos Jobs Estáticos (Segurança e Qualidade)**
  - [ ] Configurar o job de Secret Scanning utilizando a Action oficial do Gitleaks
  - [ ] Configurar o job de SAST configurando a execução do Bandit sobre a pasta `app/`
  - [ ] Configurar o job de SCA integrando o Trivy para escanear a imagem do pipeline e o arquivo `requirements.txt`
  - [ ] Adicionar checagem de estilo de código (linter como `flake8` ou `black --check`) para manter a formatação do Python
- [ ] **Etapa 3: Implementação do Job Dinâmico (Integração e Teste Ponta a Ponta)**
  - [ ] Configurar o runner do GitHub Actions (Ubuntu-latest) para suportar execução do Docker e Docker Compose
  - [ ] Criar o job de build das imagens do Docker e subir os containers locais
  - [ ] Inserir rotina de espera até que o container do MinIO atinja o status de `healthy`
  - [ ] Configurar a execução do script `bin/run-pipeline.sh` dentro do runner de CI
  - [ ] Coletar os relatórios de erros lógicos e log quantitativo gerado pelo pipeline do dbt
- [ ] **Etapa 4: Validação do Fluxo Completo de CI**
  - [ ] Simular um push de código contendo erro intencional no dbt para verificar se o CI falha e bloqueia a integração
  - [ ] Simular um push de código correto e verificar se a esteira completa é executada com sucesso
  - [ ] Atualizar o status dos testes e validações na Matriz de Rastreabilidade (RTM)

---

## 3. Lista de Arquivos Planejada (File List)

- [ ] `.github/workflows/ci.yml` -> Configuração e descrição dos jobs e etapas da esteira de CI do GitHub Actions.
- [ ] `docs/stories/story-04-ci-github-actions.md` -> Este documento de história de progresso de CI.
- [ ] `bin/ci-integration-test.sh` -> Script opcional facilitador para testar a inicialização do compose e healthcheck no runner de CI.
